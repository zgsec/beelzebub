...
// TestOllama_LLMFallback_AppliesResponseSubstitutions — YAML-authored
// llmFallback bodies (BlueSpark's litellm/vllm/ollama lures) must get
...
func TestOllama_LLMFallback_AppliesResponseSubstitutions(t *testing.T) {
	fb := &parser.LLMFallback{
		Status: 503,
		Body:   `{"error":"upstream","trace_id":"${request.uuid}","short":"${request.uuid_short}"}`,
	}
	_, body1 := ollamaLLMFallback(fb, "aurora-7b")
	...
	_, body2 := ollamaLLMFallback(fb, "aurora-7b")
	if body1 == body2 {
	...
	if fb.Body != `{"error":"upstream","trace_id":"${request.uuid}","short":"${request.uuid_short}"}` {
		t.Errorf("LLMFallback.Body mutated: %q", fb.Body)
	}
}
